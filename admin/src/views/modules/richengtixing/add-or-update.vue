	...
	<div class="addEdit-block" :style='{"padding":"30px","fontSize":"16px","color":"#666","background":"none"}'>
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="date" v-if="type!='info'" label="提醒时间" prop="tixingshijian">
					<el-date-picker
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="input" v-else-if="ruleForm.tixingshijian" label="提醒时间" prop="tixingshijian">
					<el-input v-model="ruleForm.tixingshijian" placeholder="提醒时间" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="select" v-if="type!='info'" label="律师工号" prop="lvshigonghao">
					<el-select :disabled="ro.lvshigonghao" @change="lvshigonghaoChange" v-model="ruleForm.lvshigonghao" placeholder="请选择律师工号">
						<el-option
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="input" v-else-if="ruleForm.lvshigonghao" label="律师工号" prop="lvshigonghao">
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' v-else-if="ruleForm.tixingneirong" label="提醒内容" prop="tixingneirong">
                    <span :style='{"fontSize":"16px","lineHeight":"40px","color":"inherit","fontWeight":"500","display":"inline-block"}' v-html="ruleForm.tixingneirong"></span>
                </el-form-item>
...
export default {
	...
	methods: {
		...
		init(id,type) {
            ...
            this.$http({
				url: `option/lvshi/lvshigonghao`,
				method: "get"
            }).then(({ data }) => {
				if (data && data.code === 0) {
					this.lvshigonghaoOptions = data.data;
				} else {
					this.$message.error(data.msg);
				}
            });
			...
			lvshigonghaoChange () {
				this.$http({
					url: `follow/lvshi/lvshigonghao?columnValue=`+ this.ruleForm.lvshigonghao,
					method: "get"
				}).then(({ data }) => {
					if (data && data.code === 0) {
						if(data.data.lvshixingming){
							this.ruleForm.lvshixingming = data.data.lvshixingming
						}
					} else {
						this.$message.error(data.msg);
					}
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.tixingneirong = this.ruleForm.tixingneirong.replace(reg,'../../../phpnmx94lx9/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `richengtixing/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.richengtixingCrossAddOrUpdateFlag = false;
						 this.parent.search();
