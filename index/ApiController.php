...
class ApiController extends CommonController
{
    ...
    public function download()
    {
        ...
        header('location:http://' . $_SERVER['HTTP_HOST'] . '/phpnmx94lx9/upload/' . $fileName);
        exit; 
    }
    /**
     * 获取某表的某个字段列表接口
    ...
    public function group($tablesName,$columnName){
        ...
        if ($tokens['isAdmin']!=1 || ($tokens['isAdmin']==1 && $tokens['tablename'] != "users")){
                                                                                                                                                                                                                        if($tableName == 'lvshi'){
                if($tokens['tablename'] == 'lvshi'){
                    $where .= " and `lvshigonghao` = '".$tokens['username']."' ";
                }
            }
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        if($tableName == 'yuyuexinxi'){
                if($tokens['tablename'] == 'lvshi'){
                    $where .= " and `lvshigonghao` = '".$tokens['username']."' ";
                }
            }
                ...
                if($tokens['tablename'] == 'lvshi'){
                    $where .= " and `lvshigonghao` = '".$tokens['username']."' ";
                                                                                                                                                                                                ...
                                                                                                                                                                                                if($tableName == 'richengtixing'){
                if($tokens['tablename'] == 'lvshi'){
                    $where .= " and `lvshigonghao` = '".$tokens['username']."' ";
                }
            }
                                                                                                                                                                                                                                                if($tableName == 'huiyianpai'){
                if($tokens['tablename'] == 'lvshi'){
                    $where .= " and `lvshigonghao` = '".$tokens['username']."' ";
                }
            }
                                                                                                                        if($tableName == 'xingchengxinxi'){
                if($tokens['tablename'] == 'lvshi'){
                    $where .= " and `lvshigonghao` = '".$tokens['username']."' ";
                }
            }
                                                                                                                                                                                                                                                                                                                                                                                                                                                                        if($tableName == 'feiyongxinxi'){
                if($tokens['tablename'] == 'lvshi'){
                    $where .= " and `lvshigonghao` = '".$tokens['username']."' ";
                }
            }
